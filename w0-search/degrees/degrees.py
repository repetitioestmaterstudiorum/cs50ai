...
def shortest_path(source, target):
    ...
    # add nodes of neighbors of source
    for neighbor in source_neighbors:
        node = Node(state=neighbor, parent=None) # state = (movie_id, person_id) pair
        frontier.add(node) 
        explored_person_ids.add(neighbor[1]) # add person id of each
    ...
    while True:
        ...
        # goal check
        if starbud_person_id == target:
            path = []
            while starbud:
                path.append(starbud.state)
                starbud = starbud.parent
            path.reverse()
            return path
        ...
        for neighbor in next_neighbors:
            ...
            if person_id in explored_person_ids:
                ...
                node = Node(state=neighbor, parent=starbud)
                frontier.add(node) 
                explored_person_ids.add(person_id)
